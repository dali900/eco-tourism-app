<template>
    <div class="questions p-1">
        ...
        <div class="question-header mb-3 flex justify-content-between">
            <TitleBanner title="Službena mišljenja i odgovori na pitanja"></TitleBanner>
            <div>
...
import { useQuestionStore } from '@zzs/stores/question'
...
const filters = ref({
    ...
    'question_type_id': {value: route.params.questionTypeId ? route.params.questionTypeId : null, matchMode: FilterMatchMode.EQUALS}
});
const sortItems = [
    {
        label: "Naslov",
...
//fetch filtered data
watch(() => route.params.questionTypeId, (newVal, oldVal) => {
    filters.value.question_type_id.value = newVal;
    questionStore.getQuestions({sort: sort.value, pagination: pagination.value, filters: filters.value});
})

<template>
    <div class="admin-article">
        ...
        <ArticleTypeForm v-model="showForm" :formData="formData" @saved="formSaved"></ArticleTypeForm>
        <!-- list -->
        <Card class="resource-list">
            <template #title>
                <div class="flex justify-content-between">
                    <div>
                        <span>Kategorije članaka</span>&nbsp;
                        <Button @click="openForm" icon="pi pi-plus" class="p-button-sm" v-tooltip="'Dodaj novu kategoriju članka'"></Button>
                    </div>
                </div>
            </template>
            <template #content>
                <DataTable :value="articleTypes" responsiveLayout="stack" breakpoint="960px" editMode="cell" class="edit-table" dataKey="id"
                    :loading="loading" :lazy="true"
                    @sort="onSort($event)" 
                    :paginator="true" @page="onPage($event)" :rows="perPage" :totalRecords="articleTypesTotal" :rowsPerPageOptions="[20, 50, 100]"
            ...
            <template #footer>
                <div class="flex justify-content-between">
                    <small>Total: {{articleTypesTotal}}</small>
                </div>
            </template>
        </Card>
    </div>
...
const { articleTypes, articleTypesTotal, loading } = storeToRefs(articleStore)
...
articleStore.getArticleTypes({sort: sort.value, pagination: pagination.value, filters: filters.value});
...
const deleteResource = () => {
    articleStore.deleteType(deletingResourceId.value)
        .then(() => {
            toast.add({severity:'success', summary: 'Kategorija članka obrisana.', life: 3000});

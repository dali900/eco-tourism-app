...
<template>
    <div class="admin-questions">
        <QuestionForm v-model="showForm" :formData="formData"></QuestionForm>
        <!-- list -->
        <Card class="resource-list">
            <template #title>
                <span>Pitanja i odgovori</span> <Button @click="openForm" icon="pi pi-plus" class="p-button-sm" v-tooltip="'Dodaj novo pitanje i odgovor'"></Button>
            </template>
            <template #content>
                <DataTable :value="questions" responsiveLayout="stack" breakpoint="960px" editMode="cell" class="edit-table" dataKey="id"
                    :loading="loading" :lazy="true"
                    @sort="onSort($event)" 
                    :paginator="true" @page="onPage($event)" :rows="perPage" :totalRecords="questionsTotal" :rowsPerPageOptions="[20, 50, 100]"
            ...
            <template #footer>
                <div class="flex justify-content-between">
                    <small>Total: {{questionsTotal}}</small>
                </div>
            </template>
        </Card>
    </div>
...
const questionsFilterTimer = ref(null);
...
const deleteResource = (id) => {
    questionStore.delete(id)
        .then(() => {
            toast.add({severity:'success', summary: 'Pitanje i odgovor su obrisani.', life: 3000});
        })
        .catch(() => {

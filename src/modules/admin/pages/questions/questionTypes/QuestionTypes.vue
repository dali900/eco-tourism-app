<template>
    <div class="admin-question">
        ...
        <QuestionTypeForm v-model="showForm" :formData="formData" @saved="formSaved"></QuestionTypeForm>
        <!-- list -->
        <Card class="resource-list">
            <template #title>
                <div class="flex justify-content-between">
                    <div>
                        <span>Kategorije pitanja i odgovora</span>&nbsp;
                        <Button @click="openForm" icon="pi pi-plus" class="p-button-sm" v-tooltip="'Dodaj novu kategoriju pitanja i odgovora'"></Button>
                    </div>
                </div>
            </template>
            <template #content>
                <DataTable :value="questionTypes" responsiveLayout="stack" breakpoint="960px" editMode="cell" class="edit-table" dataKey="id"
                    :loading="loading" :lazy="true"
                    @sort="onSort($event)" 
                    :paginator="true" @page="onPage($event)" :rows="perPage" :totalRecords="questionTypesTotal" :rowsPerPageOptions="[20, 50, 100]"
            ...
            <template #footer>
                <div class="flex justify-content-between">
                    <small>Total: {{questionTypesTotal}}</small>
                </div>
            </template>
        </Card>
    </div>
...
import QuestionTypeForm from './components/QuestionTypeForm.vue'
...
const { questionTypes, questionTypesTotal, loading } = storeToRefs(questionStore)
...
questionStore.getQuestionTypes({sort: sort.value, pagination: pagination.value, filters: filters.value});
...
const deleteResource = () => {
    questionStore.deleteType(deletingResourceId.value)
        .then(() => {
            toast.add({severity:'success', summary: 'Kategorija pitanja i odgovora obrisana.', life: 3000});
